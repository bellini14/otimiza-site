...
- A branch histórica `codex/restore-like-count-current-production` será usada somente como referência de comportamento. Ela não será mesclada, promovida nem publicada.
...
- Manter a precedência atual de imagem: imagem principal, primeira imagem do conteúdo e, por último, fallback.
...
- Gerar HTML social estático em `cases/<slug>/index.html` para cada case público retornado pelo Sanity.
- Combinar dados do Sanity com os dados locais atuais apenas quando título, descrição, slug ou imagem específica estiverem ausentes.
...
- Não inserir tags de imagem quando não houver URL válida.

## Estratégia de testes

- Seguir TDD: cada comportamento recuperado deve ter um teste que falhe na `main` atual antes da implementação.
- Cobrir imagem principal, fallback, transformação Sanity 1200×630, descrição e metadados Open Graph/Twitter dos posts.
