...
describe('vercel configuration', () => {
  it('serves prerendered HTML for static routes and keeps SPA fallbacks for dynamic routes', () => {
    const rawConfig = readFileSync(resolve(globalThis.process.cwd(), 'vercel.json'), 'utf8')
    ...
    expect(config.rewrites).toContainEqual({
      ...
      destination: '/silvana-bettiol.html',
    })
    expect(config.rewrites).toContainEqual({
      source: '/cases/:slug',
    ...
    expect(config.rewrites.findIndex(({ source }) => source === '/silvana-bettiol')).toBeLessThan(
      config.rewrites.findIndex(({ source }) => source === '/((?!api/).*)'),
    )
  })
